...
int dos_lmp_omp(int iargc, char* argv[], optpara opts, string atom_conc, int num_omp){
  #ifdef _OPENMP
  if(num_omp > 1)
  ...
  #pragma omp parallel for
  #endif
  for(int ij=0; ij<num_data; ij++){
    if(ij%((int)num_data/10) == 0 ) cout << "XX" << flush;
    cor[ij] = complex <double> (0.0, 0.0);
    for(int ik=0; ik<total_atom; ik++){
      for(int ii=0; ii<num_data-ij; ii++){
  ...
  for(int ij=0; ij<num_data; ij++){
    ...
    if(abs(cor[0]) > 0.0){ cor[ij] = cor[ij]/cor[0];}
      ...
      cout << "Error: Correlation Function Division by Zero. " << endl;
