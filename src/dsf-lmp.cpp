...
int dsf_lmp(int argc, char* argv[], optpara opts, string atom_conc)
{  
  /***********************************/
  ...
  {
    double m = l;
    if(wave == "220") m = part - m;
    else if(wave == "111") m = m/2.0;     
    double q[3] = {dir[0]*coe[0]*m/part/cutoff,
                   dir[1]*coe[1]*m/part/cutoff,
                   dir[2]*coe[2]*m/part/cutoff};        // wave vector[1/A]
    ...
    cout << "Calc time:" << l << "/" << part << "  ";        
    cout << "Wave vector:(" << q[0] << ", " << q[1] << ", " << q[2] << ")" << endl;
    ...
    for(int m=1; m<num_data; ++m)
    {
      ...
      for(int i=0; i<all_atom; i++)
      {
        ...
        && atom_type != atom_type_O)
        {

          displace[0] = atom[0] - zero_atom[label-1].x;
          displace[1] = atom[1] - zero_atom[label-1].y;
          displace[2] = atom[2] - zero_atom[label-1].z;
          ...
          c_la[m] += (complex<double>) (exp(-j * ((q[0]+dir_la[0]*coe[0])*(atom[0]*leng[0]   +atom[1]*tilt[0][1]+atom[2]*tilt[0][2]) + 
          ...
          c_lo[m] += (complex<double>) (exp(-j * ((q[0]+dir_lo[0]*coe[0])*(atom[0]*leng[0]   +atom[1]*tilt[0][1]+atom[2]*tilt[0][2]) + 
          ...
          c_t1[m] += (complex<double>) (exp(-j * ((q[0]+dir_t1[0]*coe[0])*(atom[0]*leng[0]   +atom[1]*tilt[0][1]+atom[2]*tilt[0][2]) + 
          ...
          c_t2[m] += (complex<double>) (exp(-j * ((q[0]+dir_t2[0]*coe[0])*(atom[0]*leng[0]   +atom[1]*tilt[0][1]+atom[2]*tilt[0][2]) + 
...
/*
          if(l == 0 && m == 1) cerr << label << " " << leng[0]
                                    ...
                                    << " re " << real(c_lo[m]) << " im " << imag(c_lo[m]) << " ab " << abs(c_lo[m]) << endl;
*/
        }
      }
